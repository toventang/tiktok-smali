package com.ss.android.ugc.aweme.filter.repository.internal.filterbox;

import com.bytedance.covode.number.Covode;
import com.google.gson.c.c;
import com.google.gson.f;
import com.google.gson.v;
import com.google.gson.w;
import com.ss.android.ugc.aweme.effectplatform.FilterEffect;
import com.ss.android.ugc.effectmanager.effect.model.Effect;
import h.f.b.l;
import java.util.Objects;

public final class FilterEffectTypeAdapterFactory implements w {
    static {
        Covode.recordClassIndex(60580);
    }

    public static final class a extends v<T> {

        /* renamed from: a  reason: collision with root package name */
        final /* synthetic */ v f95592a;

        static {
            Covode.recordClassIndex(60581);
        }

        a(v vVar) {
            this.f95592a = vVar;
        }

        @Override // com.google.gson.v
        public final T read(com.google.gson.c.a aVar) {
            return (T) this.f95592a.read(aVar);
        }

        @Override // com.google.gson.v
        public final void write(c cVar, T t) {
            v vVar = this.f95592a;
            Objects.requireNonNull(t, "null cannot be cast to non-null type com.ss.android.ugc.aweme.effectplatform.FilterEffect");
            vVar.write(cVar, t);
        }
    }

    @Override // com.google.gson.w
    public final <T> v<T> create(f fVar, com.google.gson.b.a<T> aVar) {
        l.d(fVar, "");
        l.d(aVar, "");
        if (!(!l.a(aVar.rawType, Effect.class)) || !(!l.a(aVar.rawType, com.ss.ugc.effectplatform.model.Effect.class))) {
            return new a(fVar.a(this, com.google.gson.b.a.get(FilterEffect.class)));
        }
        return null;
    }
}
