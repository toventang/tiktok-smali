package com.bytedance.jedi.arch.internal;

import com.bytedance.covode.number.Covode;
import com.bytedance.frameworks.apm.trace.MethodCollector;
import com.bytedance.jedi.arch.ag;
import f.a.aa;
import f.a.d.f;
import f.a.l.b;
import f.a.t;
import h.f.b.ab;
import h.f.b.l;
import h.k.d;
import h.z;
import java.util.Iterator;
import java.util.LinkedList;
import java.util.List;

public final class c<S> implements ag<S> {

    /* renamed from: a  reason: collision with root package name */
    final b<S> f39471a;

    /* renamed from: b  reason: collision with root package name */
    final a<S> f39472b = new a<>();

    /* renamed from: c  reason: collision with root package name */
    private final f.a.b.a f39473c;

    /* renamed from: d  reason: collision with root package name */
    private final b<z> f39474d;

    /* renamed from: e  reason: collision with root package name */
    private final t<S> f39475e;

    static {
        Covode.recordClassIndex(24289);
    }

    @Override // com.bytedance.jedi.arch.ag
    public final t<S> b() {
        return this.f39475e;
    }

    @Override // com.bytedance.jedi.arch.ag
    public final S a() {
        S i2 = this.f39471a.i();
        if (i2 == null) {
            l.a();
        }
        return i2;
    }

    static final class a<S> {

        /* renamed from: a  reason: collision with root package name */
        private final LinkedList<h.f.a.b<S, z>> f39477a = new LinkedList<>();

        /* renamed from: b  reason: collision with root package name */
        private LinkedList<h.f.a.b<S, S>> f39478b = new LinkedList<>();

        static {
            Covode.recordClassIndex(24292);
        }

        public final synchronized h.f.a.b<S, z> a() {
            MethodCollector.i(7278);
            if (this.f39477a.isEmpty()) {
                MethodCollector.o(7278);
                return null;
            }
            h.f.a.b<S, z> removeFirst = this.f39477a.removeFirst();
            MethodCollector.o(7278);
            return removeFirst;
        }

        public final synchronized List<h.f.a.b<S, S>> b() {
            MethodCollector.i(7281);
            if (this.f39478b.isEmpty()) {
                MethodCollector.o(7281);
                return null;
            }
            LinkedList<h.f.a.b<S, S>> linkedList = this.f39478b;
            this.f39478b = new LinkedList<>();
            MethodCollector.o(7281);
            return linkedList;
        }

        public final synchronized void a(h.f.a.b<? super S, z> bVar) {
            MethodCollector.i(7276);
            l.c(bVar, "");
            this.f39477a.add(bVar);
            MethodCollector.o(7276);
        }

        public final synchronized void b(h.f.a.b<? super S, ? extends S> bVar) {
            MethodCollector.i(7277);
            l.c(bVar, "");
            this.f39478b.add(bVar);
            MethodCollector.o(7277);
        }
    }

    @Override // com.bytedance.jedi.arch.ag
    public final void a(h.f.a.b<? super S, z> bVar) {
        l.c(bVar, "");
        this.f39472b.a(bVar);
        this.f39474d.onNext(z.f158842a);
    }

    @Override // com.bytedance.jedi.arch.ag
    public final void b(h.f.a.b<? super S, ? extends S> bVar) {
        l.c(bVar, "");
        this.f39472b.b(bVar);
        this.f39474d.onNext(z.f158842a);
    }

    /* JADX DEBUG: Multi-variable search result rejected for r1v0, resolved type: f.a.l.b<S> */
    /* JADX WARN: Multi-variable type inference failed */
    @Override // com.bytedance.jedi.arch.ag
    public final void c(h.f.a.b<? super S, ? extends S> bVar) {
        l.c(bVar, "");
        this.f39471a.onNext(bVar.invoke(a()));
    }

    public c(S s, aa aaVar) {
        l.c(s, "");
        l.c(aaVar, "");
        b<S> a2 = b.a((Object) s);
        l.a((Object) a2, "");
        this.f39471a = a2;
        f.a.b.a aVar = new f.a.b.a();
        this.f39473c = aVar;
        b<z> bVar = new b<>();
        l.a((Object) bVar, "");
        this.f39474d = bVar;
        t<S> c2 = a2.c();
        l.a((Object) c2, "");
        this.f39475e = c2;
        f.a.b.b a3 = bVar.a(aaVar).a(new f<z>(this) {
            /* class com.bytedance.jedi.arch.internal.c.AnonymousClass1 */

            /* renamed from: a  reason: collision with root package name */
            final /* synthetic */ c f39476a;

            static {
                Covode.recordClassIndex(24290);
            }

            {
                this.f39476a = r1;
            }

            /* JADX DEBUG: Method arguments types fixed to match base method, original types: [java.lang.Object] */
            /* JADX DEBUG: Multi-variable search result rejected for r0v4, resolved type: f.a.l.b<S> */
            /* JADX WARN: Multi-variable type inference failed */
            @Override // f.a.d.f
            public final /* synthetic */ void accept(z zVar) {
                c cVar = this.f39476a;
                while (true) {
                    h.f.a.b<S, z> a2 = cVar.f39472b.a();
                    List<h.f.a.b<S, S>> b2 = cVar.f39472b.b();
                    if (b2 != null) {
                        Object a3 = cVar.a();
                        Iterator<T> it = b2.iterator();
                        while (it.hasNext()) {
                            a3 = it.next().invoke(a3);
                        }
                        cVar.f39471a.onNext(a3);
                    }
                    if (a2 != null) {
                        a2.invoke((S) cVar.a());
                    } else {
                        return;
                    }
                }
            }
        }, new d(new h.f.a.b<Throwable, z>(this) {
            /* class com.bytedance.jedi.arch.internal.c.AnonymousClass2 */

            static {
                Covode.recordClassIndex(24291);
            }

            @Override // h.f.b.c, h.k.b
            public final String getName() {
                return "handleError";
            }

            @Override // h.f.b.c
            public final String getSignature() {
                return "handleError(Ljava/lang/Throwable;)V";
            }

            @Override // h.f.b.c
            public final d getOwner() {
                return ab.a(c.class);
            }

            /* Return type fixed from 'java.lang.Object' to match base method */
            /* JADX DEBUG: Method arguments types fixed to match base method, original types: [java.lang.Object] */
            @Override // h.f.a.b
            public final /* synthetic */ z invoke(Throwable th) {
                Throwable th2 = th;
                l.c(th2, "");
                while (true) {
                    if (th2 == null) {
                        break;
                    } else if (th2.getCause() != null) {
                        th2 = th2.getCause();
                    } else if (th2 != null) {
                        throw th2;
                    }
                }
                return z.f158842a;
            }
        }));
        l.a((Object) a3, "");
        aVar.a(a3);
    }
}
